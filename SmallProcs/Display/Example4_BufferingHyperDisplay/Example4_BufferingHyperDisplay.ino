...
ILI9341_color_16_t smallWindowMem[240*320];    // And this is a smaller portion of memory - good for 32*64 pixels
...
void setup() {
  ...
  smallWindow.xMax = TOP + (HEIGHT-1); // See, 16+0 would be one pixel, so 16+31 is actually 32 pixels wide
  smallWindow.yMax = LEFT + (WIDTH-1); // and this means 64 pixels tall.. so now our small window can be filled with 32*64 pixels
  myTFT.setWindowColorSequence(&smallWindow, (color_t)&defaultColor);
  myTFT.setWindowMemory(&smallWindow, (color_t)smallWindowMem, 240*320);  // It is really importatnt to tell the right number of pixels to this function to avoid buffer over/underflow problems
}
...
void loop() {
  ...
  ILI9341_color_16_t smallBGcolor = myTFT.rgbTo16b( 0, 0, 0 );       // Make a background color for small window
